  ...
  template: `
    <div class="page">
      <div class="card">
        ...
        <button class="btn-p" style="margin-top:16px" [disabled]="!isValid()" (click)="post()">Broadcast Announcement</button>
      </div>
        ...
        @if (announcements().length === 0) {
          ...
          <div class="ann-list">
            @for (ann of announcements(); track ann.id) {
              <div class="ann-card" style="border-left: 4px solid" [style.border-left-color]="ann.priority === 'Urgent' ? '#EF4444' : (ann.priority === 'Important' ? '#F59E0B' : '#4F46E5')">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px">
                  <h4 style="font-size:13px; font-weight:700; color:var(--text-primary)">{{ ann.title }}</h4>
                  <span class="badge" [style.background]="ann.priority === 'Urgent' ? '#FEE2E2' : (ann.priority === 'Important' ? '#FEF3C7' : '#E0E7FF')" [style.color]="ann.priority === 'Urgent' ? '#991B1B' : (ann.priority === 'Important' ? '#92400E' : '#3730A3')">{{ ann.priority }}</span>
                </div>
                <div style="font-size:11px; color:var(--text-muted); margin-bottom:8px">Target: {{ ann.sendTo }}</div>
                ...
                @if (ann.imageUrl) {
                  <div style="width:100%; max-height:120px; overflow:hidden; border-radius:6px; margin:8px 0; border:1px solid var(--border-weak); background:#F3F4F6; display:flex; justify-content:center; align-items:center">
                    <img [src]="ann.imageUrl" style="width:100%; height:auto; max-height:120px; object-fit:cover" alt="Flyer preview">
                  </div>
                }

                <p style="font-size:12px; color:var(--text-secondary)">{{ ann.message }}</p>
              </div>
            }
          </div>
        }
...
export class TeacherAnnouncementsComponent {
  ...
  message = 'Dear students, please note that our live class on Friday is rescheduled to 4:00 PM due to the national holiday. See you there!';
  imageUrl = '';
